...
contract TestUniswapV3Router is IUniswapV3SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IMonoswapV3Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function uniswapV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IMonoswapV3Pool(msg.sender).token0()
                : IMonoswapV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0
                ? amount0Delta
                : amount1Delta;

            bool zeroForOne = tokenToBePaid ==
                IMonoswapV3Pool(pools[0]).token1();
            IMonoswapV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
                zeroForOne
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IMonoswapV3Pool(msg.sender).token0())
                    .transferFrom(payer, msg.sender, uint256(amount0Delta));
            } else {
                IERC20Minimal(IMonoswapV3Pool(msg.sender).token1())
